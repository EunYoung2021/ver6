...
function BoxCar(/*int*/ x, /*int*/ y, /*Kinetic.Layer*/ layer, /*Number or Alphabetic character */value, /*boolean*/ isOutline) {
    ...
    this.init = function() {
        ...
        if(value > 0 && value < 10) {
            ...
            imageObj.src = './assets/img/img_SortOrderTrain/img' + value + '.' + imgFileExt;
            imageObj.onload = function () {
                var imgHeight = BOX_HEIGHT * 3 / 4;
                var imgWidth = imageObj.width*imgHeight/imageObj.height;
                var canvasImage= new Kinetic.Image({
                    x: imgX + BOX_WIDTH / 2 - imgWidth / 2,
                    y: imgY + BOX_HEIGHT / 2 - imgHeight / 2,
                    image: imageObj,
                    // height: BOX_HEIGHT / 2,
                });
                canvasImage.setSize(imgWidth, imgHeight);
...
var SortOrderTrain = {
    ...
    createBackground: function(/*Kinetic.Layer*/ layer) {
        ...
        imageObj.src = "./assets/img/img_SortOrderTrain/320px-Gravel_small_stones.jpg";
    ...
    setMode: function() {
        ...
        SortOrderTrain.mode = 'num';
    },
    
    setLevel: function() {
        // var lvl = parseInt($('#level').val());
        var lvl = Questions_NUM;
        if (lvl <= 3) {
            BOX_MARGIN = 100;
        } else if (lvl <= 5) {
            BOX_START = 150;
